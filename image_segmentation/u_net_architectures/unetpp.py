...
def create_unet_pp(input_size, n_labels, model_name, filter_num = [32, 64, 128, 256], stack_num_down=2, stack_num_up=2,
    ...
    if deep_supervision:
        ...
        OUT = CONV_output(X, n_labels, kernel_size=1, activation=output_activation, name=model_name)
        OUT_list = [OUT,]
        
    # model
    model = Model(inputs=[IN,], outputs=OUT_list, name=model_name)
